...
- id: a unique identifier consisting of the event's `pubKey`, `instance`, and `ordinal` joined by the `.` character.
- path: list of objects describing the path taken by the event. The first entry is the originating account.
  - pubKey: public key of the signing account
...
- pubKey: the public key of the originating account
...
In order to avoid platform-specific irregularities, like json encoding/decoding defaults, signatures should be based on the concatenation of topic, payload, instance, ordinal, each path's pubKey, timestamp, and signature, encoded as utf-8, and stored in `path` hex-encoded.
...
Schema: `{pubKey: str, handle: str, avatar: url, description: str, sharing: none|local|network}`
...
- `vote/create {record_id, record_type, score}`
- `vote/delete {gid}`
- `server/recommend {servers: [{url}]}`
- `post/create {body, parent_type, parent_id}`
- `post/delete {gid}`
- `annotation/create {post_id, body, content_type}`
- `tag/create {handle, record_type, record_id}`
- `mention/create {handle, target, record_type, record_id}`
- `subscription/create {record_type, record_id}`
- `subscription/delete {gid}`
- `group/create {name, private}`
...
This endpoint MUST accept multiple events. Schema is `{events: [event]}` where `[event]` is a list of event objects as specified above. Servers SHOULD validate that the final pubKey in each event `path` matches the pubKey provided in the `Authorization` header.
